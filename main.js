...
// Per-user keys — getters read currentUser at call time
const SK = {
...
const EXERCISE_RECS = {
  sedentary: { freq: '주 3회부터 시작',  duration: '회당 20-30분', type: '걷기 · 가벼운 유산소' },
...
function buildFeedback(prevW, newW, rate) {
  ...
  if (diff > 0 && highPerf) return {
    type: 'success', icon: '🎉',
    title: '훌륭해요! 목표 달성 중!',
    msg: `이번 주 ${diff}kg 감량 성공! 꾸준한 수행률 ${pctStr}이 빛을 발했어요. 이 페이스를 계속 유지하세요.`,
    stats: statStr,
  };
  if (diff <= 0 && highPerf) return {
    type: 'info', icon: '🤔',
  ...
  return {
    ...
    title: '다시 일어서 봐요!',
    msg: `이번 주는 조금 힘드셨군요. 수행률 ${pctStr}, 완벽하지 않아도 괜찮아요. 한 가지 습관부터 다시 시작해봐요!`,
    stats: statStr,
  };
}
...
function updateMealSubtitles() {
  MEAL_KEYS.forEach(m => {
    ...
    el.textContent = nut
      ...
      : { breakfast: '균형 잡힌 아침 식사', lunch: '영양소 균형 점심', dinner: '가볍고 건강한 저녁' }[m];
  });
}
...
function openCameraModal(mealType) {
  ...
  const labels = { breakfast: '아침 식단 분석', lunch: '점심 식단 분석', dinner: '저녁 식단 분석' };
...
function init() {
  ...
  // AI analysis buttons (breakfast / lunch / dinner)
